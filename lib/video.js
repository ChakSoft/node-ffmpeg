...
module.exports = function (filePath, settings, infoConfiguration, infoFile) {
  ...
  this.setWatermark = function (watermarkPath, settings) {
    ...
    let getSing = function (val, inverse) {
      return (
    ...
    let getHorizontalMargins = function (east, west) {
      return getSing(east, false).toString() + getSing(west, true).toString()
    }

    let getVerticalMargins = function (nord, sud) {
    ...
    if (options.hasOwnProperty('video')) {
      ...
      if (options.video.hasOwnProperty('disabled')) {
        ...
        if (options.video.hasOwnProperty('size')) {
          let newDimension = _calculateNewDimension.call(this)
   ...
   */
  let resetCommands = function (self) {
    commands = new Array()
  ...
  var _calculateNewDimension = function () {
    ...
    let fixedWidth = /([0-9]+)x\?/.exec(options.video.size)

    let fixedHeight = /\?x([0-9]+)/.exec(options.video.size)

    let percentage = /([0-9]{1,2})%/.exec(options.video.size)

    let classicSize = /([0-9]+)x([0-9]+)/.exec(options.video.size)
    ...
    if (fixedWidth) {
      ...
      let ratio = parseInt(percentage[1], 10) / 100
    ...
    if (keepAspectRatio) {
      ...
      let gcdValue = utils.gcd(width, height)
  ...
  var execCommand = function (callback, folder) {
    ...
    let onlyDestinationFile = Boolean(folder)
    ...
    // Reset commands
    resetCommands(this)
    // Execute the commands from the lists
  ...
  this.fnExtractSoundToMP3 = function (destionationFileName, callback) {
    ...
    let destinationDirName = path.dirname(destionationFileName)

    let destinationFileNameWE =
      path.basename(destionationFileName, path.extname(destionationFileName)) +
      '.mp3'
  ...
  this.fnExtractFrameToJPG = function (/* destinationFolder, settings, callback */) {
    ...
    if (
      ...
      (isNaN(settings.every_n_percentage) || settings.every_n_percentage > 100)
    ) {
      settings.every_n_percentage = null
      every_n_check++
    }
    ...
    if (settings.every_n_percentage) {
      ...
      this.addFilterComplex(
        'select=not(mod(t\\,' +
          parseInt(
            (this.metadata.duration.seconds / 100) * settings.every_n_percentage
          ) +
          '))'
      )
    }
   ...
   * Constructor
   */
  let __constructor = (function (self) {
    resetCommands(self)
  }(this))
}
