...
var ffmpeg = function (/* inputFilepath, settings, callback */) {
  ...
  let _ffmpegInfoConfiguration = function (settings) {
    ...
    utils.exec([ffmpeg.bin, '-formats', '2>&1'], settings, function (
      error,
      stdout,
      stderr
    ) {
      ...
      for (let i in codecList) {
        ...
        if (match) {
          ...
          let extension = match[2].replace(/\s/g, '')
  ...
  let _videoInfo = function (fileInput, settings) {
    ...
    // Make the call to retrieve information about the ffmpeg
    utils.exec(
      ['ffprobe', utils.addQuotes(fileInput), '2>&1'],
      settings,
      function (error, stdout, stderr) {
        // Perse output for retrieve the file info
        let filename = /from \'(.*)\'/.exec(stdout) || []

        let title = /(INAM|title)\s+:\s(.+)/.exec(stdout) || []

        let artist = /artist\s+:\s(.+)/.exec(stdout) || []

        let album = /album\s+:\s(.+)/.exec(stdout) || []

        let track = /track\s+:\s(.+)/.exec(stdout) || []

        let date = /date\s+:\s(.+)/.exec(stdout) || []

        let is_synched = /start: 0.000000/.exec(stdout) !== null

        let duration =
          /Duration: (([0-9]+):([0-9]{2}):([0-9]{2}).([0-9]+))/.exec(stdout) ||
          []

        let container = /Input #0, ([a-zA-Z0-9]+),/.exec(stdout) || []

        let video_bitrate = /bitrate: ([0-9]+) kb\/s/.exec(stdout) || []

        let video_stream =
          /Stream #([0-9\.]+)([a-z0-9\(\)\[\]]*)[:] Video/.exec(stdout) || []

        let video_codec = /Video: ([\w]+)/.exec(stdout) || []

        let resolution = /(([0-9]{2,5})x([0-9]{2,5}))/.exec(stdout) || []

        let pixel = /[SP]AR ([0-9\:]+)/.exec(stdout) || []

        let aspect = /DAR ([0-9\:]+)/.exec(stdout) || []

        let fps = /([0-9\.]+) (fps|tb\(r\))/.exec(stdout) || []

        let audio_stream =
          /Stream #([0-9\.]+)([a-z0-9\(\)\[\]]*)[:] Audio/.exec(stdout) || []

        let audio_codec = /Audio: ([\w]+)/.exec(stdout) || []

        let sample_rate = /([0-9]+) Hz/i.exec(stdout) || []

        let channels = /Audio:.* (stereo|mono)/.exec(stdout) || []

        let audio_bitrate = /Audio:.* ([0-9]+) kb\/s/.exec(stdout) || []

        let rotate = /rotate[\s]+:[\s]([\d]{2,3})/.exec(stdout) || []
        // Build return object
        let ret = {
          filename : filename[1] || '',
          title : title[2] || '',
        ...
        } else if (ret.video.resolution.w !== 0) {
